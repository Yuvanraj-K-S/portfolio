...
const SPINE_COLORS = [
  '#0a1220', '#0a0820', '#050505', '#091524', '#110225', '#00101a',
];
...
function BookCard({ project, colorIndex, index }: BookCardProps) {
    ...
    },
    {
      ...
      content: (
        <div style={{ padding: '22px 18px 16px', height: '100%', display: 'flex', flexDirection: 'column', boxSizing: 'border-box' }}>
          ...
          <p style={{ fontFamily: 'var(--font-body)', fontSize: '12px', lineHeight: 1.75, color: 'var(--text-secondary)', fontStyle: 'italic', flex: 1 }}>
  ...
  return (
    <motion.div
      ...
      onMouseLeave={() => { setIsOpen(false); setCurrentPage(0); }}
    >
      {/* Shadow */}
      <div style={{
        position: 'absolute', bottom: '-14px',
        left: isOpen ? '0%' : '6%', right: isOpen ? '-18%' : '6%',
      ...
      <div style={{
        ...
        {/* Cover */}
        <div style={{
          position: 'absolute', inset: 0, borderRadius: '0 10px 10px 0',
          ...
          <div style={{ textAlign: 'center', position: 'relative', zIndex: 1 }}>
            ...
            <div style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(18px, 3vw, 26px)', color: 'rgba(255,255,255,0.92)', lineHeight: 1.05, marginBottom: '20px', letterSpacing: '-0.01em' }}>
              {project.title}
            </div>
            <div style={{ display: 'flex', gap: '5px', justifyContent: 'center', flexWrap: 'wrap' }}>
              {project.tech.slice(0, 2).map((t, i) => (
                <span key={i} style={{ fontFamily: 'var(--font-ui)', fontSize: '7px', letterSpacing: '0.08em', color: 'rgba(255,255,255,0.4)', background: 'rgba(255,255,255,0.07)', border: '1px solid rgba(255,255,255,0.1)', padding: '2px 8px', borderRadius: '3px' }}>
                  {t}
                </span>
              ))}
            </div>
          </div>
          <div style={{ position: 'absolute', bottom: '14px', fontFamily: 'var(--font-ui)', fontSize: '7px', letterSpacing: '0.2em', color: 'rgba(255,255,255,0.18)' }}>
            HOVER TO OPEN
          </div>
        </div>
      </div>
      ...
      {isOpen && (
        <div style={{
          ...
          <button onClick={flipToPrev} disabled={currentPage === 0}
            style={{ width: '28px', height: '28px', borderRadius: '50%', background: currentPage === 0 ? 'rgba(255,255,255,0.03)' : 'rgba(255,255,255,0.1)', border: `1px solid ${currentPage === 0 ? 'rgba(255,255,255,0.05)' : 'rgba(255,255,255,0.2)'}`, color: currentPage === 0 ? 'rgba(255,255,255,0.15)' : 'rgba(255,255,255,0.7)', fontSize: '13px', cursor: currentPage === 0 ? 'default' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all 0.2s', fontFamily: 'sans-serif' }}>
            ‹
          </button>
          <div style={{ display: 'flex', gap: '5px', alignItems: 'center' }}>
            {PAGES.map((p, i) => (
              <button key={i}
                ...
                style={{ fontFamily: 'var(--font-ui)', fontSize: '6.5px', letterSpacing: '0.1em', color: i === currentPage ? 'rgba(255,255,255,0.8)' : 'rgba(255,255,255,0.25)', background: 'none', border: 'none', cursor: 'pointer', padding: '2px 4px', borderBottom: i === currentPage ? '1px solid rgba(255,255,255,0.5)' : '1px solid transparent', transition: 'all 0.2s' }}>
                {p.label}
              </button>
            ))}
          </div>
          <button onClick={flipToNext} disabled={currentPage === 2}
            style={{ width: '28px', height: '28px', borderRadius: '50%', background: currentPage === 2 ? 'rgba(255,255,255,0.03)' : 'rgba(255,255,255,0.1)', border: `1px solid ${currentPage === 2 ? 'rgba(255,255,255,0.05)' : 'rgba(255,255,255,0.2)'}`, color: currentPage === 2 ? 'rgba(255,255,255,0.15)' : 'rgba(255,255,255,0.7)', fontSize: '13px', cursor: currentPage === 2 ? 'default' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all 0.2s', fontFamily: 'sans-serif' }}>
            ›
          </button>
        </div>
      )}
...
export default function Projects() {
  ...
  return (
    ...
    >
      <div ref={headerRef} style={{ padding: '0 clamp(24px, 8vw, 120px)', marginBottom: '56px' }}>
        ...
        <motion.p
          ...
          style={{ fontFamily: 'var(--font-body)', fontSize: '13px', color: 'var(--text-muted)', marginTop: '10px', letterSpacing: '0.02em' }}
        >
      ...
      <div
        ...
        style={{
          ...
          userSelect: 'none', scrollbarWidth: 'none', msOverflowStyle: 'none',
          alignItems: 'center',
          perspective: '1200px',
        }}
      ...
      >
        {data.projects.map((project, i) => (
          <BookCard key={project.id} project={project} colorIndex={i} index={i} />
        ))}
      </div>
      ...
      <div style={{ display: 'flex', justifyContent: 'center', gap: '6px', paddingBottom: '8px' }}>
        {data.projects.map((_, i) => (
          <div key={i} style={{
            width: i === 0 ? '18px' : '5px', height: '3px', borderRadius: '2px',
